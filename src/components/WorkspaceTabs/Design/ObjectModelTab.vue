                ...
                <div class="flex h-8 bg-blue-600 text-white border text-sm">
                    <div class="flex text-center justify-center items-center flex-grow" style="position:relative">
                        <div style="position:absolute; left:0" class="ml-2">
                            <button style="float:right" @click="fixSketch(null)" :class="buttonStyle" v-if="$store.state.sketch.length">
                                Tidy up
                            </button>
                        </div>
                        sketch
                        <div style="position:absolute; right:0" class="mr-2">
                            <button v-if="$store.state.offsiteSegments.size" style="float:right" :class="buttonStyle"
                                    @click="loadOffsiteSegments(null)" :disabled="loadingOffsite || recentlyLoadedOffsite">
    ...
    export default {
        ...
        methods: {
            ...
            fixSketch(sketch = null) {
                ...
                let offsiteText = ""
                if (pastebins.length)
                    offsiteText += "pastebin > " + this.uniques(pastebins).join(', ') + Globals.___DOUBLE_LINE_BREAK___
                if (offsites.length)
                    offsiteText += "load > " + this.uniques(offsites).join(', ') + Globals.___DOUBLE_LINE_BREAK___

                let text = SketchParser.mergeDiffs([...otherSegments]).map(segment => Segment.toText(segment))
                sketch = offsiteText + text.join("\n\n").replace(/\n\s*\n/g, Globals.___DOUBLE_LINE_BREAK___)
                if(save)
